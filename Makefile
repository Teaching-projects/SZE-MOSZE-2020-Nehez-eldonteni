...
memcheck:
	valgrind --leak-check=full --error-exitcode=1 cat testing/input1.txt | ./game scenario1.json

outputTests:
	> testing/fight.txt

	cat testing/input1.txt | ./game scenario1.json | tail -1 >> testing/fight.txt
	cat testing/input2.txt | ./game scenario2.json | tail -1 >> testing/fight.txt
